...
tailrec fun findReflectionMemo(
    ...
    val result = cache13.getOrPut(listOf(field, loopIndexX, loopIndexY, index, transposed)) {
        val foo = Pair((index + 1) * if (transposed) 1 else 100, transposed)
        if (loopIndexY > field.size) {
            return Pair(-1, transposed)
        }
        if (index < field.size - 1 && (loopIndexX < 0 || loopIndexY == field.size)) {
            if (foo == original)
                println("wtf")
            else
                return foo
        }
        if ((index == field.size || loopIndexY == field.size || loopIndexX < 0) && foo != original) {
            if (transposed) {
                return Pair(-1, transposed)
            }
            ...
            if (foo != original && index < field.size - 1 && field[loopIndexX] == field[loopIndexY]) {
                return findReflectionMemo(
                    field,
                    loopIndexX - 1,
                    loopIndexY + 1,
                    index, transposed, original
                )
            } else return findReflectionMemo(
                field,
                index + 1, index + 2,
                index + 1, transposed, original
            )
    }
    return result
